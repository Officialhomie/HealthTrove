...
const RegisteredPatient = () => {
    ...
    const handleFetchPatient = () => {
        if (data) {
            setPatientAddress(data as string);
            setFetchStatus(`Patient address fetched successfully for index: ${index}`);
        } else {
            setFetchStatus(`Failed to fetch patient address for index: ${index}`);
        }
    };

    if (isLoading) return <div>Loading...</div>;
    if (isError) return <div>Error fetching patient address</div>;

    return (
        <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 max-w-md mx-auto">
            <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">Get Registered Patient Address</h2>
            <div className="mb-4">
                <input
                    type="number"
                    value={index}
            ...
            {patientAddress && (
                <div className="mt-6">
                    <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Patient Address:</h3>
                    <p className="text-lg text-gray-800 dark:text-white">{patientAddress}</p>
                </div>
            )}
        </div>
    );
