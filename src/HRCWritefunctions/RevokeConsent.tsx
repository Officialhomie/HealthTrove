...
const RevokeConsent: React.FC = () => {
    ...
    const handleRevokeConsent = async () => {
        ...
        try {
            ...
            setConsentStatus('Consent revocation request sent successfully.');
            ...
            setConsentStatus('Error revoking consent.');
        }
    };

    return (
        <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 w-full mx-auto max-w-4xl md:max-w-6xl lg:max-w-full">
            <h2 className="text-3xl font-bold mb-6 text-gray-800 dark:text-white">Revoke Doctor Consent</h2>
            {!isConnected ? (
                <div className="mb-6 p-4 bg-yellow-100 border border-yellow-400 text-yellow-700 rounded-md">
                    <p>Please connect your wallet to revoke consent.</p>
                </div>
            ) : isPatient === false ? (
                <div className="mb-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-md">
                    <p>You are not registered as a patient. Please register before revoking consent.</p>
                </div>
            ) : null}
            <form onSubmit={(e) => { e.preventDefault(); handleRevokeConsent(); }} className="grid grid-cols-1 gap-6">
                <motion.div 
                    className="mb-4"
                ...
                <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="w-full bg-red-600 text-white py-3 px-6 rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50 transition-colors duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-lg font-semibold"
                >
                    {isConnected ? 'Revoke Consent' : 'Connect Wallet'}
                </motion.button>
            </form>
